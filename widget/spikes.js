...
(function() {
    ...
    function createReviewer(name, email) {
        var reviewer = {
            id: nanoid(),
            name: name,
    ...
    function getReviewerForSpike() {
        if (currentReviewer) {
            ...
            if (currentReviewer.email) {
                reviewer.email = currentReviewer.email;
            }
            return reviewer;
        }
    ...
    function updateReviewerIndicator() {
        ...
        if (currentReviewer) {
            reviewerIndicator.innerHTML = '<span style="color:' + theme.textDim + ';">/ </span><span style="color:' + theme.text + ';cursor:pointer;" title="Click to change">' + escapeHtml(currentReviewer.name) + '</span>';
    ...
    function showNamePrompt(targetContainer, onComplete) {
        ...
        var emailField = config.collectEmail ? [
            '<input id="spikes-email-input" type="email" placeholder="Email (optional, for updates)" style="' + nameInputStyle() + 'width:100%;margin-bottom:8px;">'
        ].join('') : '';
        
        promptDiv.innerHTML = [
            '<div style="font-size:13px;font-weight:500;color:' + theme.text + ';margin-bottom:8px;">What should we call you?</div>',
            emailField,
            '<div style="display:flex;gap:8px;">',
            '  <input id="spikes-name-input" type="text" placeholder="Your name" style="' + nameInputStyle() + '">',
        ...
        function handleContinue() {
            ...
            var emailInput = promptDiv.querySelector('#spikes-email-input');
            var email = emailInput ? emailInput.value.trim() : null;
            if (name) {
                createReviewer(name, email);
                promptDiv.remove();
    ...
    function showChangeNamePrompt() {
        ...
        dialog.innerHTML = [
            '<div style="font-size:14px;font-weight:600;color:' + theme.text + ';margin-bottom:16px;"><span style="color:' + config.color + ';">/</span> Change Name</div>',
    ...
    function createModal() {
        ...
        modal.innerHTML = [
            ...
            '      <button data-rating="love" style="' + ratingBtnStyle() + '">+ Love</button>',
    ...
    function showPopover(element, elementData) {
        ...
        popover.innerHTML = [
            '<div style="margin-bottom:16px;">',
            '  <div style="font-size:11px;color:' + theme.textDim + ';margin-bottom:4px;text-transform:uppercase;letter-spacing:0.5px;"><span style="color:' + config.color + ';">/</span> Element</div>',
            '  <div style="background:' + theme.bg + ';padding:8px 10px;border-radius:6px;border-left:2px solid ' + config.color + ';font-size:12px;color:' + theme.textMuted + ';word-break:break-all;">' + escapeHtml(truncatedSelector) + '</div>',
            '  <div style="color:' + theme.textDim + ';font-size:12px;margin-top:4px;max-height:40px;overflow:hidden;font-style:italic;">"' + escapeHtml(truncatedText) + '"</div>',
            '</div>',
