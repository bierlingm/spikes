...
fn test_deploy_preserves_existing_config_sections() {
    // VAL-DEPLOY-017: deploy --force should preserve [project] and [widget] sections
    let temp_dir = tempfile::tempdir().unwrap();
    ...
    // Create a config.toml with custom [project] and [widget] sections
    let config_content = concat!(
        "[project]\n",
        "key = \"preserved-project\"\n",
        "\n",
        "[widget]\n",
        "theme = \"light\"\n",
        "position = \"top-left\"\n",
    ...
    // [project] section should be preserved
    assert!(config_after.contains("key = \"preserved-project\""), 
        "Project key should be preserved. Config:\n{}", config_after);
    ...
    // [remote] should be updated (no longer hosted, has token)
