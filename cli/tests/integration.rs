...
#[test]
fn test_deploy_preserves_existing_config_sections() {
    // VAL-DEPLOY-017: deploy --force should preserve unknown fields and comments
    // Uses toml_edit for format-preserving edits (not lossy serde round-trip)
    let temp_dir = tempfile::tempdir().unwrap();
    ...
    // - UNKNOWN field `name` in [project] (not in Config schema - would be lost by serde round-trip)
    // - A comment line that should be preserved
    // - Standard [widget] section
    ...
    std::fs::write(spikes_dir.join("feedback.jsonl"), "").unwrap();

    // Run deploy --force (bypasses hosted warning)
    cargo_bin_cmd!("spikes")
        .current_dir(temp_dir.path())
        .arg("deploy")
        .arg("cloudflare")
    ...
    // Read the config.toml after deploy
    let config_after = std::fs::read_to_string(spikes_dir.join("config.toml")).unwrap();

    // Unknown field `name` (not `key` - that's in the schema) should be preserved
    // This field would be LOST by the old serde round-trip approach
    assert!(config_after.contains("name = \"preserved-project\""), 
        "Unknown field 'name' in [project] should be preserved. Config:\n{}", config_after);

    // Comment should be preserved
    assert!(config_after.contains("my custom note"),
        "Comment should be preserved. Config:\n{}", config_after);

    // [widget] section should be preserved
    assert!(config_after.contains("theme = \"light\""),
        "Widget theme should be preserved. Config:\n{}", config_after);
    assert!(config_after.contains("position = \"top-left\""),
        "Widget position should be preserved. Config:\n{}", config_after);
    assert!(config_after.contains("color = \"#3498db\""),
        "Widget color should be preserved. Config:\n{}", config_after);
    assert!(config_after.contains("collect_email = true"),
        "Widget collect_email should be preserved. Config:\n{}", config_after);

    // [remote] should be updated with token and hosted=false
    assert!(config_after.contains("hosted = false"),
        "Hosted should be set to false. Config:\n{}", config_after);
    // Should have a token now (generated by deploy)
    assert!(config_after.contains("token = "),
        "Token should be added. Config:\n{}", config_after);

    // Verify the output is syntactically valid TOML (re-parseable)
    let parsed: Result<toml::Value, _> = config_after.parse();
    assert!(parsed.is_ok(), "Config should be valid TOML: {:?}", parsed.err());
}
